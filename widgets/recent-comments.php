...
class gates_recent_comments extends WP_Widget {

	function __construct() {
        $widget_ops = array( 'classname' => 'gates_recent_comments_widget', 'description' => __('Displays recent comments with user avatars.', 'gates') );
        parent::__construct( 'gates_recent_comments_widget', __('Recent Comments','gates'), $widget_ops );
	}
					...
					if ( $comments ) {
						foreach ( $comments as $comment ) { ?>
							...
							<li>
								...
								<a href="<?php echo get_permalink($comment->comment_post_ID); ?>#comment-<?php echo $comment->comment_ID; ?>" title="<?php _e('Comment to','gates'); echo ' ' . get_the_title($comment->comment_post_ID) . ', '; _e('posted','gates'); echo ' ' . get_the_time(get_option('date_format')); ?>">
									...
									<div class="inner">
										...
										<p class="excerpt">"<?php echo esc_attr(gates_get_comment_excerpt($comment->comment_ID, 13)); ?>"</p>
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('widget_title')); ?>"><?php  _e('Title', 'gates'); ?>:
		...
		<p>
			<label for="<?php echo $this->get_field_id('number_of_comments'); ?>"><?php _e('Number of comments to display', 'gates'); ?>:
			...
			<small>(<?php _e('Defaults to 5 if empty','gates'); ?>)</small>
		</p>
				
		<?php
	}
}
register_widget('gates_recent_comments'); ?>